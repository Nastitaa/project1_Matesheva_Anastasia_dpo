...
def is_valid_direction(direction):
    ...
    valid_directions = ['север', 'юг', 'восток', 'запад', 
                       'north', 'south', 'east', 'west']
    return direction in valid_directions


def translate_direction(direction):
    """Перевод направления на русский язык"""
    direction_map = {
        'north': 'север',
        'south': 'юг', 
        'east': 'восток',
...
def attempt_open_treasure(game_state):
    ...
    if 'treasure_key' in inventory:
        print("🗝️ Вы применяете ключ, и замок щёлкает. Сундук открыт!")
    ...
    print("🗝️ Сундук заперт. У вас нет ключа.")
